...
public class CommandFacade {
	...
	/**
	 * Constructor called by Logic which passes all arguments that might be
	 * used; this class acts as a facade to process the command types
	 * 
	 * @param taskList
	...
	/**
	 * Responsible for manipulating the undoList and determining whether the
	 * redoList should be cleared. <br>
	 ...
	 * 
	 * @param commandObj
	 *            CommandObject which contains information on the actions to
	 *            execute within the lower levels of the program
	 * @param isUndoAction
	 *            a boolean value stating if this CommandObject is a result of
	 *            an undo action
	 * @param isRedoAction
	 *            a boolean value stating if this CommandObject is a result of a
	 *            redo action
	 */
	public void run() {
	 ...
	 * storage It then adds the relevant CommandObject to the undo list or the
	 * redo list
	 */
	...
	// Calls Search function which outputs only the tasks that match the search
	// keyword
	 ...
	 * adds the relevant CommandObject to the undo list or the redo list
	 */
	...
	private void clearUndoList(Deque<CommandObject> undoList) {
	...
	// CommandObject to the undo list or the redo list
	private void processUndoForDelete(TaskObject removedTask) {
		assert removedTask != null;

		CommandObject newCommandObj = new CommandObject(commandType, removedTask, index); // MIGHT
																							// HAVE
																							// ERRORS
		if (isUndoAction) {
	...
	// Calls the Help function, which displays the user guide depending on the
	// search keyword entered (if any)
	...
	/**
	 * Method for adding a CommandObject containing add or delete to either the
	 * undoList or redoList, which is previously determined by the caller. <br>
	 * For command "add", a "delete" CommandObject will be pushed into the list.
	 * The index of the previously added TaskObject will also be added into the
	 * list to facilitate future deletion. <br>
